...
if [ -d .claude ]; then
  ...
  if [ -n "$PY" ]; then
    ...
    if "$PY" -c "import pytest" >/dev/null 2>&1; then
      "$PY" -m pytest -q scripts/ci/test_stop_gate.py scripts/ci/test_agent_sdlc.py scripts/ci/test_guardrail_diff_range.py scripts/ci/test_ci_wiring_is_real.py >/dev/null || {
        "$PY" -m pytest -q scripts/ci/test_stop_gate.py scripts/ci/test_agent_sdlc.py scripts/ci/test_guardrail_diff_range.py scripts/ci/test_ci_wiring_is_real.py; fail=1; }
