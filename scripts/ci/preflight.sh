...
if [ -d .claude ]; then
  ...
  if [ -n "$PY" ]; then
    ...
    if "$PY" -c "import pytest" >/dev/null 2>&1; then
      "$PY" -m pytest -q scripts/ci/test_stop_gate.py scripts/ci/test_agent_sdlc.py scripts/ci/test_guardrail_diff_range.py scripts/ci/test_ci_wiring_is_real.py scripts/ci/test_tool_bodies_are_guarded.py scripts/ci/test_protection_approval_is_one_commit.py scripts/ci/test_product_gate_is_a_real_boundary.py scripts/ci/test_product_gate_root_of_trust.py scripts/ci/test_deploy_identity_follows_the_pull.py >/dev/null || {
        "$PY" -m pytest -q scripts/ci/test_stop_gate.py scripts/ci/test_agent_sdlc.py scripts/ci/test_guardrail_diff_range.py scripts/ci/test_ci_wiring_is_real.py scripts/ci/test_tool_bodies_are_guarded.py scripts/ci/test_protection_approval_is_one_commit.py scripts/ci/test_product_gate_is_a_real_boundary.py scripts/ci/test_product_gate_root_of_trust.py scripts/ci/test_deploy_identity_follows_the_pull.py; fail=1; }
