...
const variants: Record<BadgeVariant, string> = {
  neutral:
    'bg-surface-2 text-text-secondary border border-[rgb(var(--border-line))]',
  subtle:
    'bg-surface-2 text-text-tertiary',
  brand:
    'bg-brand/10 text-brand',
  success:
    'bg-success-soft/12 text-success',
  warning:
  ...
  outline:
    'bg-transparent text-text-secondary border border-[rgb(var(--border-strong))]',
};
...
/** StatusDot — a single colored dot for row-level status indicators. */
...
/** Kbd — keyboard key hint, used for shortcuts. */
export function Kbd({
  children,
  className,
  ...props
