  ...
  return (
    <div className="space-y-5">
      <div className="grid grid-cols-2 gap-3 sm:grid-cols-4">
        <Stat icon={Database} label="Dataset" value={fmtNumber(totals.datasets)} />
        <Stat icon={HardDrive} label="Tổng dung lượng" value={fmtBytes(totals.bytes)} helper={`${fmtNumber(totals.rows)} dòng`} />
        <Stat icon={BarChart3} label="Biểu đồ tiêu thụ" value={fmtNumber(totals.charts)} />
        <Stat icon={Archive} label="Dataset không dùng" value={fmtNumber(totals.unused)} tone={totals.unused > 0 ? 'warning' : 'default'} />
      </div>

      <div className="flex items-center gap-2">
        <button onClick={() => setOnlyUnused(false)} className={cn('rounded-full border px-2.5 py-0.5 text-tiny font-emphasis', !onlyUnused ? 'border-brand bg-brand/10 text-brand' : 'border-[rgb(var(--border-line))] text-text-tertiary')}>Tất cả</button>
        <button onClick={() => setOnlyUnused(true)} className={cn('rounded-full border px-2.5 py-0.5 text-tiny font-emphasis', onlyUnused ? 'border-warning bg-warning/10 text-warning' : 'border-[rgb(var(--border-line))] text-text-tertiary')}>Chỉ dataset không dùng{totals.unused ? ` (${totals.unused})` : ''}</button>
      </div>

      {loading ? (
        <p className="py-10 text-center text-caption text-text-tertiary">Đang tải…</p>
      ) : view.length === 0 ? (
        <p className="py-10 text-center text-caption text-text-tertiary">{onlyUnused ? 'Mọi dataset đều đang được sử dụng.' : 'Chưa có dataset nào.'}</p>
      ) : (
          ...
          <div className="overflow-x-auto">
            <table className="app-list-table w-full divide-y divide-[rgb(var(--border-line))]">
              <thead className="bg-surface-2"><tr>
                <th className="app-list-header w-[26%]">Dataset</th>
                <th className="app-list-header">Bảng</th>
                <th className="app-list-header">Dòng</th>
                <th className="app-list-header">Dung lượng</th>
                <th className="app-list-header">Tiêu thụ</th>
                <th className="app-list-header">Làm mới</th>
                <th className="app-list-header">Giám sát</th>
              </tr></thead>
                  ...
                  <tr key={r.datasetId} className={cn('hover:bg-surface-2', r.unused && 'bg-warning/5')}>
                    <td className="app-list-cell">
                      <span className="flex items-center gap-2">
                        ...
                        {r.unused && <span className="inline-flex items-center gap-1 rounded-full bg-warning/10 px-1.5 py-0.5 text-tiny text-warning"><Archive className="h-3 w-3" />không dùng</span>}
                      </span>
                    </td>
                    <td className="app-list-cell text-caption text-text-tertiary">{r.tables}</td>
                    <td className="app-list-cell text-caption text-text-tertiary">{fmtNumber(r.rows)}</td>
                    <td className="app-list-cell text-caption text-text-tertiary">{fmtBytes(r.sizeBytes)}</td>
                    <td className="app-list-cell">
                    ...
                    <td className="app-list-cell text-tiny text-text-quaternary">{relativeTime(r.lastRefresh)}</td>
                    <td className="app-list-cell">
                      <span className="flex items-center gap-2 text-caption">
                        <span className="text-text-tertiary">{r.monitors} monitor</span>
