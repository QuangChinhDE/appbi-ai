...
export function QueryTableTab({ onAddTable, isLoading, existingTable }: QueryTableTabProps) {
  // Common
  ...
  const [vBaseTable, setVBaseTable] = useState<string>('');
  const [vColumns, setVColumns] = useState<string[]>([]);
  const [vJoins, setVJoins] = useState<JoinSpec[]>([]);
  const [vSortField, setVSortField] = useState('');
  const [vSortDir, setVSortDir] = useState<'ASC' | 'DESC'>('ASC');
  const [vLimit, setVLimit] = useState<string>('');
  ...
  // Pre-fill when editing existing table
  useEffect(() => {
    if (existingTable) {
      setSelectedDatasourceId(existingTable.datasource_id);
      setDisplayName(existingTable.display_name);
      if (existingTable.source_query) {
        setQuery(existingTable.source_query);
      }
      // Start in advanced mode for existing SQL — can't reverse-parse arbitrary SQL
      setMode('advanced');
    }
  }, [existingTable]);
  ...
  const generateSql = useCallback((): string => {
    ...
    for (const j of vJoins) {
      if (j.joinTable && j.leftKey && j.rightKey) {
        sql += `\n${j.joinType} JOIN ${j.joinTable} ON ${vBaseTable}."${j.leftKey}" = ${j.joinTable}."${j.rightKey}"`;
      }
    }
    ...
    <div className="p-6 space-y-5">
      ...
      <div className="flex justify-end pt-2 border-t">
        ...
        >
          ...
          {existingTable ? 'Lưu thay đổi' : 'Thêm bảng'}
        </button>
      </div>
    </div>
  );
