...
import { Dataset, DatasetCreate, DatasetUpdate, TransformationStep } from '@/types/api';
...
export default function DatasetEditor({
  ...
  const [transformations, setTransformations] = useState<TransformationStep[]>(initialData?.transformations || []);
  ...
  const handlePreview = () => {
    ...
    onPreview?.(dataSourceId, sqlQuery, limit);
  };
  ...
  const handleSave = () => {
    ...
    const payload = {
      ...
      transformation_version: transformations.length > 0 ? 1 : undefined,
    };

    onSave(payload);
  };

  const handleSaveTransformations = (steps: TransformationStep[]) => {
    setTransformations(steps);
    // Auto-save when transformations are updated
    const payload = {
      name,
      description: description || undefined,
      data_source_id: dataSourceId!,
  ...
  const handlePreviewTransformations = (steps: TransformationStep[]) => {
    // Preview with transformations applied
    if (!dataSourceId || !sqlQuery.trim()) return;
    setTransformations(steps);
    onPreview?.(dataSourceId, sqlQuery, previewLimit);
  };
  ...
  return (
    <div className="space-y-6">
      ...
      {/* Transform Tab */}
      {activeTab === 'transform' && mode === 'edit' && initialData && (
        <TransformTab
          dataset={{ ...initialData, transformations }}
          onSave={handleSaveTransformations}
          onPreview={handlePreviewTransformations}
        />
      )}
          ...
          <div>
            ...
            >
              {isPreviewLoading ? (
                ...
                <>
                  <Play className="w-4 h-4" />
                  Run Preview {transformations.length > 0 && '(with transformations)'}
                </>
              )}
            </button>
          </div>
          ...
          {previewResult && (
            <div>
              <h3 className="text-sm font-semibold text-gray-900 mb-3">
                Preview Results {transformations.length > 0 && `(${transformations.length} transformations applied)`}
              </h3>
              <ResultTable
