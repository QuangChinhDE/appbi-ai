...
import { RefreshCw, Pencil, Check, X, Bot, User, MessageSquare } from 'lucide-react';
import {
  useChartDescription,
...
export function ChartDescriptionPanel({ chartId, canEdit }: Props) {
  ...
  if (isLoading) {
    return <div className="p-4 text-sm text-gray-400 animate-pulse">Loading description...</div>;
  }
  ...
  const handleAddKeyword = async () => {
    ...
    if (!kw) return;
    const existing = data.insight_keywords ?? [];
    if (existing.includes(kw)) return;
    try {
      await updateMut.mutateAsync({ insight_keywords: [...existing, kw] });
      setKeywordInput('');
    } catch {
  ...
  const handleRemoveKeyword = async (kw: string) => {
    const existing = data.insight_keywords ?? [];
    try {
      await updateMut.mutateAsync({ insight_keywords: existing.filter((k) => k !== kw) });
    } catch {
    ...
    <div className="border rounded-lg p-4 space-y-4 bg-white">
      ...
      <div>
        ...
        <div className="flex flex-wrap gap-1 mb-2">
          {(data.insight_keywords ?? []).map((kw, i) => (
            <span
              key={i}
              className="flex items-center gap-1 text-xs bg-indigo-50 text-indigo-700 rounded px-2 py-0.5"
            >
              {kw}
              {canEdit && (
                <button
