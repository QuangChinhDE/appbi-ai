export type AppLanguage = 'en' | 'vi';

export type TranslationValues = Record<string, string | number>;

export type MessageCatalog = Record<string, string>;

/** A per-module catalog: paired English + Vietnamese entries that get merged into the global tables. */
export type ModuleCatalog = { en: MessageCatalog; vi: MessageCatalog };

const baseEn: MessageCatalog = {
  'common.loading': 'Loading...',
  'common.none': '— none —',
  'common.search': 'Search...',
  'common.gridView': 'Grid view',
  'common.listView': 'List view',
  'common.language': 'Language',
  'common.english': 'English',
  'common.vietnamese': 'Tiếng Việt',
  'common.cancel': 'Cancel',
  'common.close': 'Close',
  'common.save': 'Save',
  'common.delete': 'Delete',
  'common.open': 'Open',
  'common.notRunYet': 'Not run yet',
  'common.draftOnly': 'Draft only',
  'sidebar.nav.overview': 'Overview',
  'sidebar.nav.datasources': 'Data Sources',
  'sidebar.nav.datasets': 'Datasets',
  'datasets.detail.tabCaveats': 'Data caveats',
  'datasets.caveats.title': 'Data caveats',
  'datasets.caveats.subtitle': 'What the AI must state every time it explains these numbers. Scoped to this dataset — never to all of them.',
  'datasets.caveats.add': 'New caveat',
  'datasets.caveats.empty': 'No caveat defined for this dataset yet.',
  'datasets.caveats.fieldTitle': 'Short label',
  'datasets.caveats.fieldContent': 'What must always be said',
  'datasets.caveats.titlePlaceholder': 'e.g. Data ends 2018-10',
  'datasets.caveats.contentPlaceholder': 'e.g. This dataset ends in October 2018 — a question about "now" must say so.',
  'datasets.detail.tabMetrics': 'Metrics & Terms',
  'datasets.metrics.title': 'Managed metrics',
  'datasets.metrics.subtitle': 'What each number means for this dataset — the AI and every report read these definitions.',
  'datasets.metrics.add': 'New metric',
  'datasets.metrics.empty': 'No metric defined for this dataset yet.',
  'datasets.metrics.caveatsTitle': 'Data caveats',
  'datasets.metrics.caveatsSubtitle': 'Warnings always attached when these numbers are explained.',
  'datasets.metrics.caveatsEmpty': 'No caveat applies to this dataset.',
  'datasets.metrics.caveatGlobal': 'all datasets',
  'datasets.metrics.unbound': 'not bound to a dataset',
  'sidebar.nav.knowledgeHub': 'Knowledge Hub',
  'sidebar.nav.agentFlows': 'Agent Flows',
  'sidebar.nav.semantics': 'Metrics & Terms',
  'sidebar.nav.observability': 'Observability',
  'sidebar.nav.explore': 'Explore',
  'sidebar.nav.dashboards': 'Dashboards',
  'sidebar.nav.workboards': 'Workboards',
  'sidebar.nav.settings': 'Settings',
  'sidebar.group.build': 'Build',
  'sidebar.group.operate': 'Operate',
  'sidebar.group.intelligence': 'Intelligence',
  'sidebar.group.trust': 'Trust',
  'sidebar.settings.description': 'Workspace administration & access control.',
  'sidebar.settings.permissions': 'People & permissions',
  'sidebar.settings.permissionsDesc': 'Users, teams and role-based access',
  'sidebar.user.changePassword': 'Change password',
  'sidebar.user.signOut': 'Sign out',
  'sidebar.user.language': 'Interface language',
  'sidebar.collapse': 'Collapse',
  'sidebar.expand': 'Expand sidebar',
  'password.title': 'Change password',
  'password.current': 'Current password',
  'password.new': 'New password',
  'password.confirm': 'Confirm new password',
  'password.submit': 'Update password',
  'password.success': 'Password changed successfully!',
  'password.error.mismatch': 'New passwords do not match.',
  'password.error.length': 'Password must be at least 8 characters.',
  'password.error.failed': 'Failed to change password.',
  'overview.dashboards.title': 'Track the dashboards your team can refine and share',
  'overview.dashboards.description': 'Dashboards are the polished output layer of the product. Use this module to manage layouts, reopen saved boards, and continue manual edits after Explore has produced a draft.',
  'overview.dashboards.badge1': 'Layouts',
  'overview.dashboards.badge2': 'Manual editing',
  'overview.dashboards.badge3': 'Sharing',
  'overview.dashboards.saved': 'Saved dashboards',
  'overview.dashboards.savedHelper': 'Boards currently available to open and refine',
  'overview.dashboards.charts': 'Charts placed',
  'overview.dashboards.chartsHelper': 'Total chart slots already attached across dashboards',
  'overview.dashboards.updated': 'Updated 7d',
  'overview.dashboards.updatedHelper': 'Dashboards touched in the last seven days',
  'overview.datasets.title': 'Prepare reusable table datasets before you explore or automate',
  'overview.datasets.description': 'Datasets keep tables, lightweight transformations, and table context in one place. This is the staging area for Explore and AI descriptions.',
  'overview.datasets.badge1': 'Tables',
  'overview.datasets.badge2': 'Transformations',
  'overview.datasets.badge3': 'AI context',
  'overview.datasets.count': 'Datasets',
  'overview.datasets.countHelper': 'Named datasets available for table-based analysis',
  'overview.datasets.documented': 'Documented',
  'overview.datasets.documentedHelper': 'Datasets that already carry a written description',
  'overview.datasets.updated': 'Updated 7d',
  'overview.datasets.updatedHelper': 'Datasets touched in the last seven days',
  'overview.datasources.title': 'Keep every data connection visible before it feeds datasets and reports',
  'overview.datasources.description': 'Data Sources is the connection layer of the platform. Manage source configs, test access, and track which connectors are ready for live querying.',
  'overview.datasources.badge1': 'Connections',
  'overview.datasources.badge2': 'Query runner',
  'overview.datasources.badge3': 'Sync inputs',
  'overview.datasources.connections': 'Connections',
  'overview.datasources.connectionsHelper': 'Configured sources currently available to the platform',
  'overview.datasources.types': 'Source types',
  'overview.datasources.typesHelper': 'Different connector categories now represented',
  'overview.datasources.sheetsManual': 'Sheets / Manual',
  'overview.datasources.sheetsManualHelper': 'Quick read on spreadsheet and manual-entry sources',
  'overview.explore.title': 'Review saved charts before they graduate into dashboards',
  'overview.explore.description': 'Explore is where reusable chart drafts live. Use it to iterate on chart structure, keep visual building blocks organized, and promote the best results into Dashboards.',
  'overview.explore.badge1': 'Saved charts',
  'overview.explore.badge2': 'Reusable visuals',
  'overview.explore.badge3': 'Chart editing',
  'overview.explore.saved': 'Saved charts',
  'overview.explore.savedHelper': 'Chart assets currently available to open and refine',
  'overview.explore.types': 'Chart types',
  'overview.explore.typesHelper': 'Different visualization families represented right now',
  'overview.explore.updated': 'Updated 7d',
  'overview.explore.updatedHelper': 'Charts touched in the last seven days',
  'module.dashboards.title': 'Dashboards',
  'dashboards.canvas.emptyMessage': 'No charts in this dashboard. Click "Add Chart" to get started.',
  'dashboards.canvas.dragToMove': 'Drag to move',
  'dashboards.canvas.editWidget': 'Edit widget',
  'dashboards.canvas.removeWidget': 'Remove widget',
  'dashboards.canvas.dragToResize': 'Drag to resize',
  'dashboards.canvas.bringToFront': 'Bring to front',
  'dashboards.canvas.sendToBack': 'Send to back',
  'dashboards.addChart.modalTitle': 'Add Chart to Dashboard',
  'dashboards.addChart.allChartTypes': 'All chart types',
  'dashboards.addChart.chooseExisting': 'Choose Existing',
  'dashboards.addChart.createNew': 'Create New',
  'dashboards.addChart.pageLabel': 'Page: {{name}}',
  'dashboards.addChart.widthLabel': 'W',
  'dashboards.addChart.heightLabel': 'H',
  'dashboards.addChart.placementManual': 'Placement {{width}}w x {{height}}h',
  'dashboards.addChart.placementAuto': 'Size: automatic by chart type',
  'dashboards.addChart.searchPlaceholder': 'Search saved charts',
  'dashboards.addChart.scopeAll': 'All accessible',
  'dashboards.addChart.scopeMine': 'Mine only',
  'dashboards.addChart.scopeShared': 'Shared only',
  'dashboards.addChart.catalogHelper': 'Search, narrow the catalog, then inspect the full chart preview before adding it.',
  'dashboards.addChart.loadingCatalog': 'Loading chart catalog...',
  'dashboards.addChart.chartsAvailable': '{{count}} charts available',
  'dashboards.addChart.chartAvailable': '{{count}} chart available',
  'dashboards.addChart.alreadyAddedNotice': 'Charts already added to this page are hidden from this picker.',
  'dashboards.addChart.noMatches': 'No charts match the current search or filters.',
  'dashboards.addChart.sectionRecommendedTitle': 'Recommended for this dashboard',
  'dashboards.addChart.sectionRecommendedHelper': 'Charts connected to datasets already used in this dashboard.',
  'dashboards.addChart.sectionReuseTitle': 'Already used on another page',
  'dashboards.addChart.sectionReuseHelper': 'Reusable charts already attached elsewhere in this dashboard.',
  'dashboards.addChart.sectionSharedTitle': 'Shared with you',
  'dashboards.addChart.sectionSharedHelper': 'Charts owned by someone else but available for this dashboard.',
  'dashboards.addChart.sectionOtherTitle': 'Other charts',
  'dashboards.addChart.sectionOtherHelper': 'Everything else you can access.',
  'dashboards.addChart.shared': 'Shared',
  'dashboards.addChart.usedOn': 'Used on {{pages}}',
  'dashboards.addChart.updatedOn': 'Updated {{date}}',
  'dashboards.addChart.selected': 'Selected',
  'dashboards.addChart.select': 'Select',
  'dashboards.addChart.previewLabel': 'Preview',
  'dashboards.addChart.previewEmpty': 'Pick a chart from the catalog to review it here before adding it.',
  'dashboards.addChart.savedChartPreview': 'Saved chart preview',
  'dashboards.addChart.multiSelectedTitle': '{{count}} charts selected',
  'dashboards.addChart.singleSelectPrompt': 'Pick 1 chart to preview',
  'dashboards.addChart.multiSelectedHelper': 'All will be added at once (default parameters). To preview and adjust parameters, select exactly 1 chart.',
  'dashboards.addChart.singleSelectHelper': 'Click a chart in the list on the left — the preview renders fully with data and parameters before you add it.',
  'dashboards.addChart.instanceParameters': 'Instance parameters',
  'dashboards.addChart.instanceParametersHelper': 'Leave blank to keep the chart default values.',
  'dashboards.addChart.dateRangePlaceholder': 'YYYY-MM-DD..YYYY-MM-DD',
  'dashboards.addChart.optionalPlaceholder': 'optional',
  'dashboards.addChart.dateRangeHint': 'Use `start..end` or `start,end`.',
  'dashboards.addChart.addNotice': 'The chart will be added at the top. You can drag or resize it after adding.',
  'dashboards.addChart.addNoticeWithPage': 'The chart will be added at the top of {{name}}. You can drag or resize it after adding.',
  'dashboards.addChart.backToPicker': 'Back to picker',
  'dashboards.addChart.saveAndAdd': 'Save Chart & Add',
  'dashboards.addChart.adding': 'Adding...',
  'dashboards.addChart.addChart': 'Add Chart',
  'dashboards.addChart.addCharts': 'Add {{count}} charts',
  'dashboards.addChart.previewError': 'Failed to load chart preview.',
  'module.datasets.title': 'Datasets',
  'module.datasources.title': 'Data Sources',
  'module.explore.title': 'Explore',
  'action.newDashboard': 'New Dashboard',
  'action.newDataset': 'New Dataset',
  'action.newDataSource': 'New Data Source',
  'action.newChart': 'New Chart',
};

const baseVi: MessageCatalog = {
  'common.loading': 'Đang tải...',
  'common.none': '— không chọn —',
  'common.search': 'Tìm kiếm...',
  'common.gridView': 'Dạng lưới',
  'common.listView': 'Dạng danh sách',
  'common.language': 'Ngôn ngữ',
  'common.english': 'English',
  'common.vietnamese': 'Tiếng Việt',
  'common.cancel': 'Hủy',
  'common.close': 'Đóng',
  'common.save': 'Lưu',
  'common.delete': 'Xóa',
  'common.open': 'Mở',
  'common.notRunYet': 'Chưa chạy',
  'common.draftOnly': 'Chỉ có draft',
  'sidebar.nav.overview': 'Tổng quan',
  'sidebar.nav.datasources': 'Nguồn dữ liệu',
  'sidebar.nav.datasets': 'Dataset',
  'datasets.detail.tabCaveats': 'Lưu ý dữ liệu',
  'datasets.caveats.title': 'Lưu ý về dữ liệu',
  'datasets.caveats.subtitle': 'Điều AI buộc phải nói mỗi lần giải thích các con số này. Chỉ áp dụng cho bộ dữ liệu này, không phải cho tất cả.',
  'datasets.caveats.add': 'Thêm lưu ý',
  'datasets.caveats.empty': 'Chưa có lưu ý nào cho bộ dữ liệu này.',
  'datasets.caveats.fieldTitle': 'Nhãn ngắn',
  'datasets.caveats.fieldContent': 'Điều luôn phải nêu',
  'datasets.caveats.titlePlaceholder': 'ví dụ: Dữ liệu dừng ở 2018-10',
  'datasets.caveats.contentPlaceholder': 'ví dụ: Bộ dữ liệu này kết thúc tháng 10/2018 — câu hỏi về "hiện tại" phải nêu rõ.',
  'datasets.detail.tabMetrics': 'Chỉ số & Thuật ngữ',
  'datasets.metrics.title': 'Chỉ số quản trị',
  'datasets.metrics.subtitle': 'Từng con số nghĩa là gì với bộ dữ liệu này — AI và mọi báo cáo đều đọc các định nghĩa ở đây.',
  'datasets.metrics.add': 'Thêm chỉ số',
  'datasets.metrics.empty': 'Chưa định nghĩa chỉ số nào cho bộ dữ liệu này.',
  'datasets.metrics.caveatsTitle': 'Lưu ý về dữ liệu',
  'datasets.metrics.caveatsSubtitle': 'Cảnh báo luôn được kèm theo khi giải thích các con số này.',
  'datasets.metrics.caveatsEmpty': 'Không có lưu ý nào áp dụng cho bộ dữ liệu này.',
  'datasets.metrics.caveatGlobal': 'mọi bộ dữ liệu',
  'datasets.metrics.unbound': 'chưa gắn dataset',
  'sidebar.nav.knowledgeHub': 'Knowledge Hub',
  'sidebar.nav.agentFlows': 'Agent Flows',
  'sidebar.nav.semantics': 'Chỉ số & Thuật ngữ',
  'sidebar.nav.observability': 'Observability',
  'sidebar.nav.explore': 'Khám phá',
  'sidebar.nav.dashboards': 'Dashboard',
  'sidebar.nav.workboards': 'Workboard',
  'sidebar.nav.settings': 'Cài đặt',
  'sidebar.group.build': 'Xây dựng',
  'sidebar.group.operate': 'Vận hành',
  'sidebar.group.intelligence': 'Trí tuệ',
  'sidebar.group.trust': 'Tin cậy',
  'sidebar.settings.description': 'Quản trị workspace & phân quyền truy cập.',
  'sidebar.settings.permissions': 'Người dùng & Phân quyền',
  'sidebar.settings.permissionsDesc': 'Người dùng, nhóm và phân quyền theo vai trò',
  'sidebar.user.changePassword': 'Đổi mật khẩu',
  'sidebar.user.signOut': 'Đăng xuất',
  'sidebar.user.language': 'Ngôn ngữ giao diện',
  'sidebar.collapse': 'Thu gọn',
  'sidebar.expand': 'Mở rộng thanh bên',
  'password.title': 'Đổi mật khẩu',
  'password.current': 'Mật khẩu hiện tại',
  'password.new': 'Mật khẩu mới',
  'password.confirm': 'Xác nhận mật khẩu mới',
  'password.submit': 'Cập nhật mật khẩu',
  'password.success': 'Đổi mật khẩu thành công!',
  'password.error.mismatch': 'Mật khẩu mới không khớp.',
  'password.error.length': 'Mật khẩu phải có ít nhất 8 ký tự.',
  'password.error.failed': 'Không thể đổi mật khẩu.',
  'overview.dashboards.title': 'Theo dõi các dashboard mà đội ngũ có thể tiếp tục tinh chỉnh và chia sẻ',
  'overview.dashboards.description': 'Dashboards là lớp đầu ra đã được hoàn thiện của sản phẩm. Hãy dùng module này để quản lý bố cục, mở lại các board đã lưu và tiếp tục chỉnh tay sau khi Explore tạo ra bản nháp.',
  'overview.dashboards.badge1': 'Bố cục',
  'overview.dashboards.badge2': 'Chỉnh tay',
  'overview.dashboards.badge3': 'Chia sẻ',
  'overview.dashboards.saved': 'Dashboard đã lưu',
  'overview.dashboards.savedHelper': 'Các board hiện có thể mở lại và tinh chỉnh',
  'overview.dashboards.charts': 'Chart đã đặt',
  'overview.dashboards.chartsHelper': 'Tổng số vị trí chart đang được gắn trên các dashboard',
  'overview.dashboards.updated': 'Cập nhật 7 ngày',
  'overview.dashboards.updatedHelper': 'Dashboard được chạm tới trong 7 ngày gần đây',
  'overview.datasets.title': 'Chuẩn bị dataset bảng dùng lại trước khi khám phá hoặc tự động hóa',
  'overview.datasets.description': 'Dataset lưu các bảng, biến đổi nhẹ và context của bảng trong cùng một nơi. Đây là khu vực staging cho Explore và AI Description.',
  'overview.datasets.badge1': 'Bảng',
  'overview.datasets.badge2': 'Biến đổi',
  'overview.datasets.badge3': 'Ngữ cảnh AI',
  'overview.datasets.count': 'Dataset',
  'overview.datasets.countHelper': 'Dataset đặt tên sẵn cho phân tích dựa trên bảng',
  'overview.datasets.documented': 'Đã mô tả',
  'overview.datasets.documentedHelper': 'Dataset đã có mô tả bằng văn bản',
  'overview.datasets.updated': 'Cập nhật 7 ngày',
  'overview.datasets.updatedHelper': 'Dataset được chạm tới trong 7 ngày gần đây',
  'overview.datasources.title': 'Theo dõi mọi kết nối dữ liệu trước khi chúng cấp nguồn cho dataset và report',
  'overview.datasources.description': 'Data Sources là lớp kết nối của nền tảng. Hãy quản lý cấu hình nguồn, kiểm tra kết nối và theo dõi connector nào đã sẵn sàng cho truy vấn trực tiếp.',
  'overview.datasources.badge1': 'Kết nối',
  'overview.datasources.badge2': 'Chạy truy vấn',
  'overview.datasources.badge3': 'Đầu vào sync',
  'overview.datasources.connections': 'Kết nối',
  'overview.datasources.connectionsHelper': 'Nguồn dữ liệu hiện được cấu hình trong hệ thống',
  'overview.datasources.types': 'Loại nguồn',
  'overview.datasources.typesHelper': 'Số nhóm connector khác nhau đang có mặt',
  'overview.datasources.sheetsManual': 'Sheets / Manual',
  'overview.datasources.sheetsManualHelper': 'Tổng quan nhanh về nguồn Google Sheets và nhập tay',
  'overview.explore.title': 'Xem lại các chart đã lưu trước khi nâng cấp chúng thành dashboard',
  'overview.explore.description': 'Explore là nơi lưu các bản nháp chart có thể tái sử dụng. Hãy dùng nó để lặp nhanh cấu trúc chart, tổ chức các khối hình ảnh và đẩy kết quả tốt nhất sang Dashboards.',
  'overview.explore.badge1': 'Chart đã lưu',
  'overview.explore.badge2': 'Visual tái sử dụng',
  'overview.explore.badge3': 'Chỉnh sửa chart',
  'overview.explore.saved': 'Chart đã lưu',
  'overview.explore.savedHelper': 'Tài sản chart hiện có thể mở lại và tinh chỉnh',
  'overview.explore.types': 'Loại chart',
  'overview.explore.typesHelper': 'Số nhóm visualization đang được dùng',
  'overview.explore.updated': 'Cập nhật 7 ngày',
  'overview.explore.updatedHelper': 'Chart được chạm tới trong 7 ngày gần đây',
  'module.dashboards.title': 'Dashboards',
  'dashboards.canvas.emptyMessage': 'Chưa có biểu đồ nào trong dashboard này. Bấm "Thêm biểu đồ" để bắt đầu.',
  'dashboards.canvas.dragToMove': 'Kéo để di chuyển',
  'dashboards.canvas.editWidget': 'Sửa widget',
  'dashboards.canvas.removeWidget': 'Xóa widget',
  'dashboards.canvas.dragToResize': 'Kéo để thay đổi kích thước',
  'dashboards.canvas.bringToFront': 'Đưa lên trước',
  'dashboards.canvas.sendToBack': 'Đưa ra sau',
  'dashboards.addChart.modalTitle': 'Thêm biểu đồ vào Dashboard',
  'dashboards.addChart.allChartTypes': 'Tất cả loại biểu đồ',
  'dashboards.addChart.chooseExisting': 'Chọn có sẵn',
  'dashboards.addChart.createNew': 'Tạo mới',
  'dashboards.addChart.pageLabel': 'Trang: {{name}}',
  'dashboards.addChart.widthLabel': 'R',
  'dashboards.addChart.heightLabel': 'C',
  'dashboards.addChart.placementManual': 'Bố cục {{width}}r x {{height}}c',
  'dashboards.addChart.placementAuto': 'Kích thước: tự động theo loại chart',
  'dashboards.addChart.searchPlaceholder': 'Tìm biểu đồ đã lưu',
  'dashboards.addChart.scopeAll': 'Tất cả truy cập được',
  'dashboards.addChart.scopeMine': 'Chỉ của tôi',
  'dashboards.addChart.scopeShared': 'Chỉ được chia sẻ',
  'dashboards.addChart.catalogHelper': 'Tìm kiếm, lọc danh mục, rồi xem trước đầy đủ biểu đồ trước khi thêm.',
  'dashboards.addChart.loadingCatalog': 'Đang tải danh mục biểu đồ...',
  'dashboards.addChart.chartsAvailable': 'Có {{count}} biểu đồ',
  'dashboards.addChart.chartAvailable': 'Có {{count}} biểu đồ',
  'dashboards.addChart.alreadyAddedNotice': 'Các biểu đồ đã thêm vào trang này được ẩn khỏi danh sách chọn.',
  'dashboards.addChart.noMatches': 'Không có biểu đồ nào khớp với tìm kiếm hoặc bộ lọc hiện tại.',
  'dashboards.addChart.sectionRecommendedTitle': 'Gợi ý cho dashboard này',
  'dashboards.addChart.sectionRecommendedHelper': 'Các biểu đồ kết nối với dataset đã dùng trong dashboard này.',
  'dashboards.addChart.sectionReuseTitle': 'Đã dùng ở trang khác',
  'dashboards.addChart.sectionReuseHelper': 'Biểu đồ tái sử dụng đã gắn ở nơi khác trong dashboard này.',
  'dashboards.addChart.sectionSharedTitle': 'Được chia sẻ với bạn',
  'dashboards.addChart.sectionSharedHelper': 'Biểu đồ do người khác sở hữu nhưng có sẵn cho dashboard này.',
  'dashboards.addChart.sectionOtherTitle': 'Biểu đồ khác',
  'dashboards.addChart.sectionOtherHelper': 'Tất cả những gì khác mà bạn truy cập được.',
  'dashboards.addChart.shared': 'Được chia sẻ',
  'dashboards.addChart.usedOn': 'Dùng ở {{pages}}',
  'dashboards.addChart.updatedOn': 'Cập nhật {{date}}',
  'dashboards.addChart.selected': 'Đã chọn',
  'dashboards.addChart.select': 'Chọn',
  'dashboards.addChart.previewLabel': 'Xem trước',
  'dashboards.addChart.previewEmpty': 'Chọn một biểu đồ từ danh mục để xem lại tại đây trước khi thêm.',
  'dashboards.addChart.savedChartPreview': 'Xem trước biểu đồ đã lưu',
  'dashboards.addChart.multiSelectedTitle': '{{count}} chart đã chọn',
  'dashboards.addChart.singleSelectPrompt': 'Chọn 1 chart để xem trước',
  'dashboards.addChart.multiSelectedHelper': 'Tất cả sẽ được add cùng lúc (parameters mặc định). Để xem trước + chỉnh parameters, chọn đúng 1 chart.',
  'dashboards.addChart.singleSelectHelper': 'Click vào 1 chart ở danh sách bên trái — preview render đầy đủ với data + parameters trước khi add.',
  'dashboards.addChart.instanceParameters': 'Tham số riêng',
  'dashboards.addChart.instanceParametersHelper': 'Để trống để giữ giá trị mặc định của biểu đồ.',
  'dashboards.addChart.dateRangePlaceholder': 'YYYY-MM-DD..YYYY-MM-DD',
  'dashboards.addChart.optionalPlaceholder': 'tùy chọn',
  'dashboards.addChart.dateRangeHint': 'Dùng `start..end` hoặc `start,end`.',
  'dashboards.addChart.addNotice': 'Biểu đồ sẽ được thêm ở trên cùng. Bạn có thể kéo hoặc đổi kích thước sau khi thêm.',
  'dashboards.addChart.addNoticeWithPage': 'Biểu đồ sẽ được thêm ở trên cùng của {{name}}. Bạn có thể kéo hoặc đổi kích thước sau khi thêm.',
  'dashboards.addChart.backToPicker': 'Quay lại danh sách chọn',
  'dashboards.addChart.saveAndAdd': 'Lưu biểu đồ & Thêm',
  'dashboards.addChart.adding': 'Đang thêm...',
  'dashboards.addChart.addChart': 'Thêm biểu đồ',
  'dashboards.addChart.addCharts': 'Thêm {{count}} biểu đồ',
  'dashboards.addChart.previewError': 'Không tải được bản xem trước biểu đồ.',
  'module.datasets.title': 'Dataset',
  'module.datasources.title': 'Data Sources',
  'module.explore.title': 'Explore',
  'action.newDashboard': 'Tạo dashboard',
  'action.newDataset': 'Tạo dataset',
  'action.newDataSource': 'Tạo nguồn dữ liệu',
  'action.newChart': 'Tạo chart',
};

// Per-module catalogs. Add a new module file under `./catalog/`, export its
// `ModuleCatalog`, and register it here. Every key MUST be defined in both `en`
// and `vi` (the fallback chain en -> key still protects against typos).
import { dashboardsCatalog } from './catalog/dashboards';
import { dashboardsDetailCatalog } from './catalog/dashboards-detail';
import { dashboardsModalsCatalog } from './catalog/dashboards-modals';
import { dashboardsAiBotCatalog } from './catalog/dashboards-aibot';
import { dashboardsThemeCatalog } from './catalog/dashboards-theme';
import { datasetsCatalog } from './catalog/datasets';
import { exploreCatalog } from './catalog/explore';
import { governCatalog } from './catalog/govern';
import { agentFlowsCatalog } from './catalog/agent-flows';
import { workboardsCatalog } from './catalog/workboards';
import { datasourcesCatalog } from './catalog/datasources';
import { observabilityCatalog } from './catalog/observability';
import { publicCatalog } from './catalog/public';
import { settingsCatalog } from './catalog/settings';
import { sharedCatalog } from './catalog/shared';

const moduleCatalogs: ModuleCatalog[] = [
  dashboardsCatalog,
  dashboardsDetailCatalog,
  dashboardsModalsCatalog,
  dashboardsAiBotCatalog,
  dashboardsThemeCatalog,
  datasetsCatalog,
  exploreCatalog,
  governCatalog,
  agentFlowsCatalog,
  workboardsCatalog,
  datasourcesCatalog,
  observabilityCatalog,
  publicCatalog,
  settingsCatalog,
  sharedCatalog,
];

function mergeCatalogs(base: MessageCatalog, pick: (c: ModuleCatalog) => MessageCatalog): MessageCatalog {
  return moduleCatalogs.reduce<MessageCatalog>(
    (acc, catalog) => Object.assign(acc, pick(catalog)),
    { ...base },
  );
}

export const en: MessageCatalog = mergeCatalogs(baseEn, (c) => c.en);
export const vi: MessageCatalog = mergeCatalogs(baseVi, (c) => c.vi);

export const messages: Record<AppLanguage, MessageCatalog> = { en, vi };
