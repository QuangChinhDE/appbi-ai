...
async def lifespan(app: FastAPI):
    ...
    # Build the reranker's ONNX session now. The first inference costs ~1.9s
    # against ~700ms warm, and paying that in whoever asks the first question of
    # the day turns a component we chose into a mystery slow answer. Absent model
    # returns False and logs once — a deployment fact, not an error.
    try:
        from app.services.dashboard_ai_bot.doc_rerank_semantic import warm as rerank_warm
        rerank_warm()
    except Exception:  # noqa: BLE001 — retrieval works without it
        logging.getLogger(__name__).warning("doc reranker warm-up failed", exc_info=True)
    ...
    # process — without this they'd be stuck in "running" forever. Skipped
    # while the module is disabled: with the router unmounted no new sync run
    # can be created, so there is nothing left for this to reclaim.
