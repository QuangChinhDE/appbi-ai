...
- POST /integrations/embed/resolve  (Bearer token) — get-or-create a deduped,
  filter-locked embed link and return a fresh rotating 256-char embed URL.
- POST /integrations/clients        (admin) — provision a client; its Bearer
  token is returned exactly once.
- GET  /integrations/clients        (admin) — list clients (no tokens).
...
class EmbedResolveRequest(BaseModel):
    ...
    # Safety gate: embedding the WHOLE report (no row-scoping) must be an
    # explicit choice. With no filters and full_report=False the request is
    # rejected (400) so a caller can't accidentally leak the entire dataset by
    # forgetting to send the per-viewer scope.
...
class IntegrationClientCreated(BaseModel):
    id: uuid.UUID
    token: str  # the Bearer token — shown ONCE, store it on your server
    name: str
    max_ttl_seconds: int
...
def create_integration_client(
    ...
    client = IntegrationClient(
        id=uuid.uuid4(),
        token_hash=hash_client_token(raw_token),  # only the hash is stored
...
# ---------------------------------------------------------------------------
# Bearer-authenticated embed resolution (called by external software A)
# ---------------------------------------------------------------------------
...
def resolve_embed_link(
    ...
    # Safety gate: no filters + no explicit full_report → reject, so a caller
    # can't accidentally embed the ENTIRE report by forgetting the per-viewer
    # scope. To intentionally embed the whole report, pass full_report=true.
    ...
    # Empty filters (full_report) → an unscoped link.
    ...
    logger.info(
        "embed_resolve client=%s name=%s dashboard=%s filter_hash=%s ttl=%s ip=%s",
        client.id, client.name, dash.id, filter_hash[:12], ttl,
