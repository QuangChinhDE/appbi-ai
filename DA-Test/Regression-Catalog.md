...
| A turn that read NO data and stated 7–9 figures anyway was recorded `ok` — the Runs header read "100% TRẢ LỜI ĐƯỢC · 0 LỖI" above a viewer notice saying those numbers have no source (live runs 464–466 on `revenue_v2`) | `_verify_figures` returns early on an empty ledger reporting `no_evidence`, while the downgrade added in `3c3945b` keyed off `grounding.all_evidence_unresolved` — a rule reading a key the producing branch never set. Each half had a passing test | the empty-ledger branch now reports `no_evidence_after_read`, from `state.question_grounding` (written by every report-read step at entry), and the downgrade is bounded to runs that attempted a read — a flow with no read step promised no data (`0cf0906`) | ✅ `test_question_grounding.py` — 4 cases: the downgrade, the bound, a structural test tying producer to consumer, and a numberless refusal staying `ok`; `test_a_matching_branch_stays_a_clean_success` caught the unbounded first version |
| The same turns STILL read `ok` after the first fix, and the viewer notice itself was wrong — it told the reader their figures had no source about data the model had just been shown (live runs 471/472/475) | the `report_read` step was `reused` (`run_policy=when_stale`), so `_run_node` returns before the handler body; `_reuse` hydrates the node's VARIABLE — deliberately — but restored no evidence and no grounding, so the verifier judged the answer against an empty ledger | `_reuse` now re-adds the stored value's numbers to the ledger and carries its `selection` forward: a reused read genuinely read, one turn earlier (`d8486ca`) | ✅ `test_question_grounding.py` — ledger restored, grounding restored, and the negative: reuse invents nothing it was not given |
| The Runs tab said "Câu trả lời không dẫn nguồn nào" on every reused turn, including answers whose text carries `[chart:683]` | `history.no_citation` is derived from the recorded `citations`, and `_reuse` restored the ledger and the selection but not the citations — same root cause as the row above, one field further | `_reuse` re-cites from the stored value's own `charts` (id + title, the fields the handler cites with), deduplicated against this turn (`d64e764`) | ✅ `test_question_grounding.py` — citations restored + no duplicate on re-entry |
| That citation restore shipped DEAD — it appended nothing at all, and the warning stayed on every reused turn | it read `entry["id"]`; a chart entry has no `id` (`data.py` builds `{"chart_id": chart_id}`). It passed its own test because the FIXTURE beside it also said `"id"` — code and test agreeing with each other about a payload neither had checked against the producer | read `chart_id`; the fixture now carries the handler's real shape and a test asserts it does, naming both places that must change together. Dedup compares `kind` too (`f0f7eb4`) | ✅ `test_the_fixture_is_the_shape_the_read_handler_builds` — the missing lock |
| A REUSED report read handed the verifier a different ledger from the same read when it originally ran: `charts[].chart_id`, `scope.read` and `scope.available` became checkable evidence, so a claim of 41 verified against the id of chart 41 — and the real figures were MISSING, since rows sit 7 levels deep and `add_evidence` stops at 6 | `_reuse` restored provenance with a blanket `state.add_evidence(value)` over the whole stored output, while the original harvests only `_call` results (`inspect_filters`, per-chart `summary`/`data`, the KPI figure) and routes the rest through `_route_call` on purpose | `restore_report_read_provenance()` in `handlers/data.py`, beside the producer it has to mirror; walks the data-bearing sub-payloads only | ✅ `test_reuse_evidence_equivalence.py` — original-vs-reused set equality for evidence, labels, citations and grounding, built by running the REAL handler; mutation-controlled (reinstating the blanket restore turns 4 red) |
| A chart's own id — and a RELATED chart's id — were checkable evidence on every run, not just reused ones, so a hallucinated "41 đơn" verified against chart 41 and shipped as sourced | `_route_call` keeps a chart LISTING out of the ledger, but the data tools echo the id back inside their own result (`get_chart_data` → `_ok({"chart_id": ...})`, insight pack opens with `chart_id` + `related[].chart_id`), and those go through `_call`. The invariant held only where nobody had looked | `RunState.add_evidence` skips numeric values under identifier keys — keyed on what the value IS, not a list of numeric field names; titles/names stay, `_unknown_labels` needs them | ✅ `test_reuse_evidence_equivalence.py` with stubs rebuilt from the REAL tool returns — the hand-written ones omitted `chart_id` and made the claim-vs-id test green against a payload the product never produces |
| 5 @critical E2E tests skipped themselves on every CI run, reported as `37 passed / 5 skipped` | 4 of them needed only a link with an ACTIVE binding, which a freshly migrated CI database has none of — a FIXTURE gap that read as a suite property. The 5th asserts a run came back `ok`/`partial`, which needs a provider; its guard tested `status >= 500` and would have stopped firing the moment a link existed, since the endpoint answers 200 with a failed run | `seed_e2e_binding.py` creates one link+binding through the real service (not INSERTs), wired into `e2e.yml`; the provider-dependent one skips on `E2E_NO_MODEL`, declared by the workflow | ✅ observed against a backend run with NO model key: 4 pass, 1 skips. E2E 41 passed / 1 skipped |
...
| Off-domain question answered with a full Olist business summary ("thời tiết Hà Nội hôm nay" → GMV, orders, AOV, ratings) | TWO causes. (a) `_charts_for_question` resolved `none`, labelled `fell_back_to: report_order` and then returned the WHOLE allowed list — observability was mistaken for correctness. (b) the resolver accepted any `search_business_assets` hit, so one incidental token reported `status: "semantic"` on unrelated charts (run 446) | question mode reads nothing on none/ambiguous and exposes `unsupported`/`needs_clarification`; semantic matches need ≥2 shared terms (or all terms of a short question), scored with the discover pack's own scorer; grounding propagated to `RunState.question_grounding` so verified numbers from unresolved evidence cannot look healthy; a broken lookup stays a separate state and still degrades | ✅ `test_question_grounding.py` (behavioural eval) + `test_asset_resolution.py` false-positive cases — both mutation-tested |
| Resolver failure still read the whole report; grounding recorded but inert; reader notice emitted but never rendered on `/d/[token]` | (a) `lookup_failed` degraded to report order — fail-open with a label. (b) `question_grounding` existed and nothing consumed it, and `evidence` is a flat float list so "which step produced this figure" was unanswerable. (c) the public SSE union never listed `type: "result"`, so `applyEvent` had no branch, `onResult` was declared/passed/never called, and every runtime notice was discarded one function short of message state | lookup failure reads 0 charts with `resolution_unavailable` (neither "relevant" nor "unsupported"); `evidence_sources` added as the minimum provenance, flagged only when EVERY contributor is unresolved; `result` added to the SSE contract and handled | ✅ `test_question_grounding.py`, `test_no_evidence_no_confident_number.py`, `qa:answer-parity` rule 6 — all mutation-tested |
| Vietnamese "giá trị đơn hàng trung bình" refused as ambiguous | it cleared the strength threshold against TWO governed metrics sharing "trung bình" (AOV 5 terms, review score 3), and any tie was ambiguous | a concept matching strictly more of the question wins; a genuine tie stays ambiguous. No threshold was lowered | ✅ `test_cross_lingual_resolution.py` incl. the tie case |
| **"Bang nào có doanh thu cao nhất?" answered "health_beauty"** — a product CATEGORY presented as a state, with a figure and no caveat; reproduced live twice in a row | TWO causes, and the first invalidated a previous "fixed". (a) `_charts_on_table` decided `measure_match`/`dimension_match` by substring over `json.dumps(chart.config)`, and a real config carries the dataset's whole column list in `baseFilters` — so on report 67 `resolve_chart_candidates(measure=total_revenue, dimension=customer_state)` returned 684/685/686 as `complete` and the ONE chart grouped by state (687) as incomplete, with the identical set for `dimension=product_category_name_english`. The unit test passed because its fixture used a tidy hand-built config. (b) nothing gated the DIRECT agent path: the report-read step ran in report-order mode (a valid authoring choice), so the agent called `rank_values` on a chart it picked itself | matcher reads the structured grouping key from `roleConfig` via `chart_meta[id]["fields"]`; new `tools/dimension_gate.py` refuses a grouped call whose chart groups by something other than the requested dimension (`dimension_mismatch`, non-retryable, with a `resolve_chart_candidates` recovery), wired one line after the capability gate in `registry.execute`; the requested dimension falls back to the charts' own breakdowns because the deployment declares 24 semantic fields and ALL of them `kind="measure"`; `get_chart_data` is gated too after the model, once refused, read rows from the monthly chart and answered with a MONTH; an unsatisfied gap reaches the verdict as `partial` + a reader notice | ✅ `test_requested_dimension_is_not_substituted.py` (41 cases, both mutation controls red) + LIVE D2 6/6 runs declining honestly — no category, no month
| Capability shortlist lost answers live (23/30 vs 27/30) and `find_capability` was never used; default 40 hid it by never routing | ranking mixed a generic node prompt into the question; hidden catalogue sat in the system prompt | schema-budget routing, question-ranked (prompt tilts ≤30%), catalogue inside `find_capability`, load by need/name (`c7224fc1`) | ✅ `test_capability_routing_eval.py` (52 intents, 600 distractors) |
| Agent found its evidence, asked for one more tool on its last model call, run ended "hết số lượt gọi mô hình" | no reservation for later steps; last call still offered tools | `runtime/reserve.py` + `Budget.reserve`; last available call answer-only (`95e331e8`) | ✅ `test_budget_always_reaches_an_answer.py` |
| Language reminder never read by the answering model | appended after the reasoning loop | phase-placed instructions (after_tools / final) (`95e331e8`) | ✅ `test_instruction_lifecycle.py` |
| Loop reported 5 results when only 1 iteration ran | body's last output survived from the previous iteration | cleared per iteration (`95e331e8`) | ✅ replay `13_budget_exhausted` |
| Tool step's evidence recorded under the PREVIOUS step's key | `run_tool` never set `evidence_source` | executor sets it for every leaf (`7b5a3e00`) | ✅ `test_compute_typed_contract.py` (mutation) |
| Disabled/unshared Skill still ran from a pin | pin = unconditional; sharing only checked at save/publish | per-version lifecycle + access re-check in `invoke_skill` (`7b5a3e00`) | ✅ `test_skill_lifecycle_and_revocation.py` |
| Text Skill prose / compute `literals[0]` certified by a later compute | trust judged only by root provenance | `evidence_paths` — only vouched fields are trusted (`d2968cca`) | ✅ `test_adversarial_review_v3_closure.py` |
